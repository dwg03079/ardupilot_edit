...
/// @file    AP_Nav_G1.h
/// @brief   G1 Control algorithm. This is a instance of an
/// AP_Navigation class
 ...
 *  - Ability to use loiter radius smaller than G1 length
 */
...
class AP_Nav_G1 : public AP_Navigation {
public:
    AP_Nav_G1(AP_AHRS& ahrs, const AP_TECS* tecs)
        : _ahrs(ahrs)
        , _tecs(tecs)
    {
    ...
    float crosstrack_error_integrator(void) const override { return _G1_xtrack_i; }
    ...
    // set the default NAVG1_PERIOD
    void set_default_period(float period) {
        _G1_period.set_default(period);
    }
    ...
    // this supports the NAVG1_* user settable parameters
...
private:
    ...
    // G1 reference point (+ve to right)
    float _latAccDem;

    // G1 tracking distance in meters which is dynamically updated
    float _G1_dist;

    // Status which is true when the vehicle has started circling the WP
    bool _WPcircle;

    // bearing angle (radians) to G1 point
    float _nav_bearing;
    ...
    // G1 tracking loop period (sec)
    AP_Float _G1_period;
    // G1 tracking loop damping ratio
    AP_Float _G1_damping;
    ...
    float _G1_xtrack_i = 0;
    AP_Float _G1_xtrack_i_gain;
    float _G1_xtrack_i_gain_prev = 0;
