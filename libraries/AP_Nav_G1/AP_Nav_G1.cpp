...
const AP_Param::GroupInfo AP_Nav_G1::var_info[] = {
    // @Param: PERIOD
    // @DisplayName: G1 control period
    // @Description: Period in seconds of G1 tracking loop. This parameter is the primary control for agressiveness of turns in auto mode. This needs to be larger for less responsive airframes. The default of 20 is quite conservative, but for most RC aircraft will lead to reasonable flight. For smaller more agile aircraft a value closer to 15 is appropriate, or even as low as 10 for some very agile aircraft. When tuning, change this value in small increments, as a value that is much too small (say 5 or 10 below the right value) can lead to very radical turns, and a risk of stalling.
    // @Units: s
    // @Range: 1 60
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("PERIOD",    0, AP_Nav_G1, _G1_period, 17),

    // @Param: DAMPING
    // @DisplayName: G1 control damping ratio
    // @Description: Damping ratio for G1 control. Increase this in increments of 0.05 if you are getting overshoot in path tracking. You should not need a value below 0.7 or above 0.85.
    // @Range: 0.6 1.0
    // @Increment: 0.05
    // @User: Advanced
    AP_GROUPINFO("DAMPING",   1, AP_Nav_G1, _G1_damping, 0.75f),

    // @Param: XTRACK_I
    // @DisplayName: G1 control crosstrack integrator gain
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_GROUPINFO("XTRACK_I",   2, AP_Nav_G1, _G1_xtrack_i_gain, 0.02),
...
//Modified to use PD control for circle tracking to enable loiter radius less than G1 length
 ...
 */
int32_t AP_Nav_G1::get_yaw_sensor() const
{
    if (_reverse) {
...
/*
  ...
  update_*() operation
 */
float AP_Nav_G1::lateral_acceleration(void) const
{
    return _latAccDem;
}

int32_t AP_Nav_G1::nav_bearing_cd(void) const
{
    return wrap_180_cd(RadiansToCentiDegrees(_nav_bearing));
}

int32_t AP_Nav_G1::bearing_error_cd(void) const
{
    return RadiansToCentiDegrees(_bearing_error);
}

int32_t AP_Nav_G1::target_bearing_cd(void) const
{
 ...
 */
float AP_Nav_G1::turn_distance(float wp_radius) const
{
    wp_radius *= sq(_ahrs.get_EAS2TAS());
    return MIN(wp_radius, _G1_dist);
}
 ...
 */
float AP_Nav_G1::turn_distance(float wp_radius, float turn_angle) const
{
...
float AP_Nav_G1::loiter_radius(const float radius) const
{
...
bool AP_Nav_G1::reached_loiter_target(void)
{
    return _WPcircle;
}
 ...
 */
void AP_Nav_G1::_prevent_indecision(float& Nu)
{
...
void AP_Nav_G1::update_waypoint(const Location& prev_WP, const Location& next_WP, float dist_min)
{
    ...
    // Calculate G1 gain required for specified damping
    float K_G1 = 4.0f * _G1_damping * _G1_damping;
    ...
    // Calculate the G1 length required for specified period
    // 0.3183099 = 1/1/pipi
    _G1_dist = MAX(0.3183099f * _G1_damping * _G1_period * groundSpeed, dist_min);
    ...
    //and further than G1 distance from WP A. Then use WP A as the G1 reference point
    //Otherwise do normal G1 guidance
    float WP_A_dist = A_air.length();
    float alongTrackDist = A_air * AB;
    if (WP_A_dist > _G1_dist && alongTrackDist / MAX(WP_A_dist, 1.0f) < -0.7071f)
    {
        ...
        _nav_bearing = atan2f(-A_air_unit.y, -A_air_unit.x); // bearing (radians) from AC to G1 point
    }
        ...
        Nu = atan2f(xtrackVel, ltrackVel);
        _nav_bearing = atan2f(-B_air_unit.y, -B_air_unit.x); // bearing (radians) from AC to G1 point
    }
     ...
     //Calculate Nu2 angle (angle of velocity vector relative to line connecting waypoints)
        ...
        //Calculate Nu1 angle (Angle to G1 reference point)
        float sine_Nu1 = _crosstrack_error / MAX(_G1_dist, 0.1f);
        ...
        if (_G1_xtrack_i_gain <= 0 || !is_equal(_G1_xtrack_i_gain.get(), _G1_xtrack_i_gain_prev)) {
            _G1_xtrack_i = 0;
            _G1_xtrack_i_gain_prev = _G1_xtrack_i_gain;
        }
        else if (fabsf(Nu1) < radians(5)) {
            _G1_xtrack_i += Nu1 * _G1_xtrack_i_gain * dt;

            // an AHRS_TRIM_X=0.1 will drift to about 0.08 so 0.1 is a good worst-case to clip at
            _G1_xtrack_i = constrain_float(_G1_xtrack_i, -0.1f, 0.1f);
        }
        ...
        Nu = Nu1 + Nu2;
        _nav_bearing = wrap_PI(atan2f(AB.y, AB.x) + Nu1);   // bearing (radians) from AC to G1 point
    }
    ...
    _latAccDem = K_G1 * groundSpeed * groundSpeed / _G1_dist * sinf(Nu);
...
// update G1 control for loitering
void AP_Nav_G1::update_loiter(const Location& center_WP, float radius, int8_t loiter_direction)
{
    Location _current_loc;
    ...
    float omega = (6.2832f / _G1_period);
    float Kx = omega * omega;
    float Kv = 2.0f * _G1_damping * omega;

    // Calculate G1 gain required for specified damping (used during waypoint capture)
    float K_G1 = 4.0f * _G1_damping * _G1_damping;
    ...
    // Calculate the G1 length required for specified period
    // 0.3183099 = 1/pi
    _G1_dist = 0.3183099f * _G1_damping * _G1_period * groundSpeed;
    ...
    //Calculate lat accln demand to capture center_WP (use G1 guidance law)
    float latAccDemCap = K_G1 * groundSpeed * groundSpeed / _G1_dist * sinf(Nu);
...
// update G1 control for heading hold navigation
void AP_Nav_G1::update_heading_hold(int32_t navigation_heading_cd)
{
    // Calculate normalised frequency for tracking loop
    const float omegaA = 4.4428f / _G1_period; // sqrt(2)*pi/period
    ...
    // Calculate time varying control parameters
    _G1_dist = groundSpeed / omegaA; // G1 distance is adjusted to maintain a constant tracking loop frequency
...
// update G1 control for level flight on current heading
void AP_Nav_G1::update_level_flight(void)
{
    // copy to _target_bearing_cd and _nav_bearing
