...
/*
  backend driver for AOA from a I2C SDP3X sensor

  with thanks to https://github.com/PX4/Firmware/blob/master/src/drivers/sdp3x_AOA
 */
...
AP_AOA_SDP3X::AP_AOA_SDP3X(AP_AOA &_frontend, uint8_t _instance) :
 ...
 */
bool AP_AOA_SDP3X::_send_command(uint16_t cmd)
{
...
bool AP_AOA_SDP3X::init()
{
    ...
    _dev->register_periodic_callback(20000,
                                     FUNCTOR_BIND_MEMBER(&AP_AOA_SDP3X::_timer, void));
    return true;
}

// read the values from the sensor. Called at 50Hz
void AP_AOA_SDP3X::_timer()
{
 ...
 */
float AP_AOA_SDP3X::_correct_pressure(float press)
{
    float sign = 1.0f;
    
    // fix for tube order
    AP_AOA::pitot_tube_order tube_order = get_tube_order();
    switch (tube_order) {
    case AP_AOA::PITOT_TUBE_ORDER_NEGATIVE:
        press = -press;
        sign = -1.0f;
        break;
    case AP_AOA::PITOT_TUBE_ORDER_POSITIVE:
        break;
    //case AP_AOA::PITOT_TUBE_ORDER_AUTO:
    default:
        /*if (press < 0.0f) {
            sign = -1.0f;
            press = -press;
    ...
    // pressure correction. We need to do this so the AOA ratio
...
bool AP_AOA_SDP3X::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AP_AOA_SDP3X::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
 ...
 */
bool AP_AOA_SDP3X::_crc(const uint8_t data[], uint8_t size, uint8_t checksum)
{
