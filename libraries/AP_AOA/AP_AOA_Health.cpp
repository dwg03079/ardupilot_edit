...
void AP_AOA::check_sensor_failures()
{
...
void AP_AOA::check_sensor_ahrs_wind_max_failures(uint8_t i)
{
#ifndef HAL_BUILD_AP_PERIPH
    ...
    if (gps.status() < AP_GPS::GPS_Status::GPS_OK_FIX_3D) {
        // GPS speed can't be trusted, re-enable AOA as a fallback
        if ((param[i].use == 0) && (state[i].failures.param_use_backup == 1)) {
            GCS_SEND_TEXT(MAV_SEVERITY_NOTICE, "AOA sensor %d, Re-enabled as GPS fall-back", i+1);
    ...
    // check for AOA consistent with wind and vehicle velocity using the EKF
    uint32_t age_ms;
    ...
    if (is_positive(_wind_gate) && (AP_AOA::OptionsMask::USE_EKF_CONSISTENCY & _options) != 0) {
    ...
    const float speed_diff = fabsf(state[i].AOA-gps.ground_speed());
    ...
    if (param[i].use > 0) {
        if (((AP_AOA::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_DO_DISABLE & _options) != 0) &&
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "AOA sensor %d failure. Disabling", i+1);
        ...
        if (is_positive(wind_warn) && (speed_diff > wind_warn) && ((now_ms - state[i].failures.last_warn_ms) > 15000)) {
            ...
            GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "AOA %d warning %0.1fm/s air to gnd speed diff", i+1, speed_diff);
        }

    // if Re-Enable options is allowed, and sensor is disabled but was previously enabled, and is probably healthy
    } else if (((AP_AOA::OptionsMask::ON_FAILURE_AHRS_WIND_MAX_RECOVERY_DO_REENABLE & _options) != 0) &&
                (state[i].failures.param_use_backup > 0) && 
                (state[i].failures.health_probability > RE_ENABLE_PROB_THRESH_OK)) {

        GCS_SEND_TEXT(MAV_SEVERITY_NOTICE, "AOA sensor %d now OK. Re-enabled", i+1);
