...
AP_AOA_NMEA::AP_AOA_NMEA(AP_AOA &_frontend, uint8_t _instance) :
    AP_AOA_Backend(_frontend, _instance)
{
}

bool AP_AOA_NMEA::init()
{
    const AP_SerialManager& serial_manager = AP::serialmanager();

    _uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_AOA, 0);
    if (_uart == nullptr) {
        return false;
    }
    ...
    _uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_AOA, 0));
...
bool AP_AOA_NMEA::get_AOA(float &AOA)
{
    if (_uart == nullptr) {
        return false;
    }
...
bool AP_AOA_NMEA::get_temperature(float &temperature)
{
    if (_uart == nullptr) {
        return false;
    }
...
bool AP_AOA_NMEA::decode(char c)
{
    switch (c) {
    case ',':
...
bool AP_AOA_NMEA::decode_latest_term()
{
