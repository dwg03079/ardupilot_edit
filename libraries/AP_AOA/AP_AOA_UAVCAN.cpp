...
AP_AOA_UAVCAN::DetectedModules AP_AOA_UAVCAN::_detected_modules[];
HAL_Semaphore AP_AOA_UAVCAN::_sem_registry;

// constructor
AP_AOA_UAVCAN::AP_AOA_UAVCAN(AP_AOA &_frontend, uint8_t _instance) :
    AP_AOA_Backend(_frontend, _instance)
{}

void AP_AOA_UAVCAN::subscribe_msgs(AP_DroneCAN* ap_dronecan)
{
    if (ap_dronecan == nullptr) {
        return;
    }

    if (Canard::allocate_sub_arg_callback(ap_dronecan, &handle_AOA, ap_dronecan->get_driver_index()) == nullptr) {
        AP_BoardConfig::allocation_error("AOA_sub");
    }
...
AP_AOA_Backend* AP_AOA_UAVCAN::probe(AP_AOA &_frontend, uint8_t _instance, uint32_t previous_devid)
{
    WITH_SEMAPHORE(_sem_registry);

    AP_AOA_UAVCAN* backend = nullptr;
    ...
    for (uint8_t i = 0; i < AOA_MAX_SENSORS; i++) {
        if (_detected_modules[i].driver == nullptr && _detected_modules[i].ap_dronecan != nullptr) {
            ...
            backend = new AP_AOA_UAVCAN(_frontend, _instance);
            if (backend == nullptr) {
                AP::can().log_text(AP_CANManager::LOG_INFO,
                                   LOG_TAG,
                                   "Failed register UAVCAN AOA Node %d on Bus %d\n",
                ...
                AP::can().log_text(AP_CANManager::LOG_INFO,
                                   LOG_TAG,
                                   "Registered UAVCAN AOA Node %d on Bus %d\n",
                                   _detected_modules[i].node_id,
                                   _detected_modules[i].ap_dronecan->get_driver_index());
...
AP_AOA_UAVCAN* AP_AOA_UAVCAN::get_uavcan_backend(AP_DroneCAN* ap_dronecan, uint8_t node_id)
{
    if (ap_dronecan == nullptr) {
        return nullptr;
    }
...
void AP_AOA_UAVCAN::handle_AOA(AP_DroneCAN *ap_dronecan, const CanardRxTransfer& transfer, const uavcan_equipment_air_data_RawAirData &msg)
{
    WITH_SEMAPHORE(_sem_registry);

    AP_AOA_UAVCAN* driver = get_uavcan_backend(ap_dronecan, transfer.source_node_id);

    if (driver != nullptr) {
        WITH_SEMAPHORE(driver->_sem_AOA);
...
#if AP_AOA_HYGROMETER_ENABLE
void AP_AOA_UAVCAN::handle_hygrometer(AP_DroneCAN *ap_dronecan, const CanardRxTransfer& transfer, const dronecan_sensors_hygrometer_Hygrometer &msg)
{
    WITH_SEMAPHORE(_sem_registry);
...
bool AP_AOA_UAVCAN::init()
{
    // always returns true
    return true;
}

bool AP_AOA_UAVCAN::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(_sem_AOA);
...
bool AP_AOA_UAVCAN::get_temperature(float &temperature)
{
 ...
 */
bool AP_AOA_UAVCAN::get_hygrometer(uint32_t &last_sample_ms, float &temperature, float &humidity)
{
