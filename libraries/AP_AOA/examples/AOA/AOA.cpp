...
// create an AHRS object for get_AOA_max
AP_AHRS ahrs;
...
void setup()
{
    ...
    // set AOA pin to 65, enable and use to true
    set_object_value(&AOA, AOA.var_info, "PIN", 65);
    ...
    // initialize AOA
    // Note AOA.set_log_bit(LOG_BIT) would need to be called in order to enable logging
    AOA.init();
...
void loop(void)
{
    ...
    if ((AP_HAL::millis() - timer) > 100) {
        ...
        // print temperature and AOA to console
        hal.console->printf("AOA %5.2f temperature %6.2f healthy = %u\n",
